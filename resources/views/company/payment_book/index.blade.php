...
<script>
    $(document).ready(function() {
        ...
        const editbaseUrl = "{{ route('company.payment.book.edit', ['id' => ':id']) }}";
        const printbaseUrl = "{{ route('company.payment.book.print', ['id' => ':id']) }}";

        const table = $('#variationTable').DataTable({
            processing: true,
            ajax: {
                url: "{{ route('company.payment.book.getall') }}",
                type: 'GET',
            },
            columns: [
                {
        ...
        window.deletePurchase = function(purchaseBookId) {
            ...
            Swal.fire({
                title: "Are you sure?",
                text: "Do you want to delete this Payment Book voucher and update the stock?",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    const url = '{{ route("company.payment.book.destroy", ":purchaseBookId") }}'.replace(":purchaseBookId", purchaseBookId);
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (response.success) {
                                setFlash("success", "Payment Book voucher deleted successfully and stock updated.");
                            } else {
                                setFlash("error", "There was an issue deleting the Payment Book voucher. Please contact your system administrator.");
                            }
                            table.ajax.reload();
                        },
